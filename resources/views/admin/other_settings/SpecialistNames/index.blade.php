        ...
        <div class="my-auto">
            <div class="d-flex">
                <h4 class="content-title mb-0 my-auto">الرئيسية </h4><span
                    class="text-muted mt-1 tx-13 mr-2 mb-0">/ التخصصات المهنية     </span>
            </div>
        </div>
    </div>
    <!-- breadcrumb -->
            ...
            <div class="card">
                <div class="card-header">
                    <button data-target="#add_job_name_model"
                            data-toggle="modal" class="btn btn-primary btn-sm">  اضافة تخصص  <i
                            class="fa fa-plus"></i>
                    </button>
                    @include('admin.other_settings.SpecialistNames.add')
                </div>
                        ...
                        <div class="table-responsive">
                            <table class="table text-md-nowrap" id="example2">
                                ...
                                @php
                                    $i = 1;
                                @endphp
                                @foreach($allspecialists as $special)
                                    <tr>
                                        <td> {{$i++}} </td>
                                        <td> {{$special['name']}} </td>
                                        ...
                                        @endif  </td>
                                        <td>
                                            <button data-target="#edit_model_{{$special['id']}}"
                                                    data-toggle="modal" class="btn btn-primary btn-sm">  <i
                                                    class="fa fa-edit"></i>
                                            </button>
                                            <button data-target="#delete_model_{{$special['id']}}"
                                    ...
                                    @include('admin.other_settings.SpecialistNames.edit')
                                    @include('admin.other_settings.SpecialistNames.delete')
                                @endforeach
                                </tbody>
                            </table>
                        </div>
